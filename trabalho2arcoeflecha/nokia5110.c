...
void line_lcd(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2){
	uint8_t step ;
float dx = (x2 - x1);
  float dy = (y2 - y1);
  if (fabs(dx) >= fabs(dy))
 ...
 void circle_lcd(uint8_t x_centre, uint8_t y_centre, uint8_t r){

	int x = r, y = 0;
	nokia_lcd_set_pixel(x+x_centre, y+y_centre,1);

		 nokia_lcd_set_pixel(x+x_centre, -y+y_centre,1);
		 nokia_lcd_set_pixel(y+x_centre, x+y_centre,2);
		nokia_lcd_set_pixel(-y+x_centre, x+y_centre,3);
	int P = 1 - r;
	while (x > y)
	{ 
		y++;
		...
		// in the other octants after translation
		 ...
		 nokia_lcd_set_pixel(x+x_centre, y+y_centre,1);
		 nokia_lcd_set_pixel(-x+x_centre, y+y_centre,2);
		nokia_lcd_set_pixel(x+x_centre, -y+y_centre,1);
		nokia_lcd_set_pixel(-x+x_centre, y+y_centre,1);
		...
		if (x != y)
		{
	     nokia_lcd_set_pixel(y+x_centre, x+y_centre,1);
		 nokia_lcd_set_pixel(-y+x_centre, x+y_centre,1);
		 nokia_lcd_set_pixel(y+x_centre, -x+y_centre,1);
		 nokia_lcd_set_pixel(-y+x_centre, x+y_centre,1);
